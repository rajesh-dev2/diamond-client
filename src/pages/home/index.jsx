...
export default function Home() {
  ...
  const tabs = sports?.length ? mapSportsToTabs(sports) : defaultTabs

  const activeEtid = sports?.find((sport) => sport.name === activeTab)?.etid
  const { data: events } = useGetEventsQuery(activeEtid, { skip: !activeEtid })

  const renderTabContent = () => {
    if (activeTab === 'Horse Racing') {
      return <RacingTable racingData={horseRacingData} />
    }
        ...
        <ul className="nav nav-tabs md:hidden! menu-tabs">
          <li className="nav-item">
            <Link className="nav-link" to="/aviator-list">
              <svg id="Layer_2" data-name="Layer 2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 457.6 277.4" className="w-[26px] h-[16px] inline-block mr-1">
                <defs><style>{`.cls-1 { fill: #e40539; }`}</style></defs>
          ...
          <li className="nav-item"><Link className="nav-link" to="/sports-book/33">LOTTERY</Link></li>
          <li className="nav-item"><Link className="nav-link active" to="/home">SPORTS</Link></li>
          <li className="nav-item"><Link className="nav-link" to="/casino-list/LC/4">OUR CASINO</Link></li>
          <li className="nav-item"><Link className="nav-link" to="/live-casino-list/CS/24">LIVE CASINO</Link></li>
          <li className="nav-item"><Link className="nav-link" to="/slot-list">SLOTS</Link></li>
          <li className="nav-item"><Link className="nav-link" to="/fantasy-list">FANTASY</Link></li>
        </ul>

        {/* Sports Navigation Tabs */}
        <SportsTab tabs={tabs} activeTab={activeTab} onTabChange={setActiveTab} />
